...
import { Banknote, BarChart3, Timer, Users, TrendingUp, TrendingDown, Minus } from "lucide-react";
...
async function getStats() {
  ...
  const revAujourdhui = sessionsAujourdhui.reduce((s, x) => s + (x.montant ?? 0), 0);
  const revHier = sessionsHier.reduce((s, x) => s + (x.montant ?? 0), 0);
  const revMois = sessionsMois.reduce((s, x) => s + (x.montant ?? 0), 0);

  const dureesMoyenne =
    sessionsSemaine.filter((s) => s.dureeMinutes).reduce((acc, s) => acc + (s.dureeMinutes ?? 0), 0) /
    (sessionsSemaine.filter((s) => s.dureeMinutes).length || 1);
  ...
  const machineNom = Object.fromEntries(machines.map((m) => [m.id, m.nom]));

  const chartData: { jour: string; revenus: number }[] = [];
  for (let i = 6; i >= 0; i--) {
    const d = startOfDay(subDays(now, i));
    const next = startOfDay(subDays(now, i - 1));
    const s = sessionsSemaine.filter((x) => new Date(x.debut) >= d && new Date(x.debut) < next);
    chartData.push({
      jour: d.toLocaleDateString("fr-FR", { weekday: "short" }),
      revenus: s.reduce((acc, x) => acc + (x.montant ?? 0), 0),
    });
  }
  ...
  return {
    ...
    machinesTotal,
    clientsTotal,
    topMachines: topMachines.map((m) => ({ nom: machineNom[m.machineId] ?? "?", count: m._count.id })),
    chartData,
  };
}
    ...
    <div className="space-y-8 max-w-7xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <StatCard
          ...
          value={formatMontant(stats.revAujourdhui)}
          sub={`${delta >= 0 ? "+" : ""}${delta.toFixed(0)}% vs hier`}
        ...
        <StatCard
          ...
          value={formatMontant(stats.revMois)}
          sub="mois en cours"
          trend="flat"
          icon={BarChart3}
          accent="#7C3AED"
        ...
        <StatCard
          ...
          value={String(stats.sessionsEnCours)}
          sub={`${stats.machinesTotal} postes au total`}
          trend="flat"
          icon={Timer}
          accent="#06B6D4"
        ...
        <StatCard
          ...
          sub={`Durée moy. ${formatDuree(Math.round(stats.dureesMoyenne))}`}
          trend="flat"
          icon={Users}
          accent="#A78BFA"
...
function StatCard({
  title, value, sub, trend, icon: Icon, accent, glowColor,
}: {
  title: string;
  value: string;
  sub: string;
    ...
    >
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-1.5" style={{ color: trendColor }}>
          <TrendIcon size={13} />
          <span className="text-xs font-medium" style={{ fontFamily: "'Chakra Petch', sans-serif" }}>
            {sub}
          </span>
        </div>
      </div>
